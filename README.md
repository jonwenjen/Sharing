...
## 權限控管
- 能進入帳本的人：建立者、已綁定的帳本成員、用**邀請連結**加入過的人、帳本所連結 LINE 群組的成員（由機器人向 LINE 驗證）。
- 其他人即使知道帳本網址也會看到「沒有權限」。邀請連結外洩時，到帳本設定按「重設邀請連結」，舊連結立即失效（已加入者不受影響）。
- **管理員**可看到並進入所有帳本：首頁點右上角頭像複製自己的 LINE ID，填入 `worker/wrangler.toml` 的 `ADMIN_USER_IDS`（多位用逗號分隔）後部署。
- 首頁帳本卡片會標示：連結的 LINE 群組名稱、建立者、「我建立的」「管理員檢視」「已封存」。
- 注意：LINE Login channel 與 Messaging API channel 必須在**同一個 Provider** 底下，群組成員驗證才會成功。
- 匯款資訊只有綁定的本人能修改；刪除帳本僅限建立者；資料庫欄位會在後端啟動時自動補齊。
