...
with sync_playwright() as p:
    ...
    shot('15-member-sync')
    pg.keyboard.press('Escape'); pg.wait_for_timeout(300)
    pg.get_by_role('button', name='回帳本列表').click(); expect(pg.locator('.home-title')).to_be_visible()
    pg.locator('.ledger-card', has_text='東京五日遊 2026').click(); pg.get_by_role('tab', name='成員').click()
    pg.locator('.member', has_text='小安').click()
    expect(pg.get_by_role('dialog').get_by_label('帳號', exact=True)).to_have_value('9999-8888')
    pg.keyboard.press('Escape'); pg.wait_for_timeout(300)
    pg.get_by_role('button', name='回帳本列表').click()
    # 不同幣別分開結算
    expect(pg.locator('.home-title')).to_be_visible()
    pg.locator('.ledger-card', has_text='東京五日遊 2026').click()
    expect(pg.locator('.ticket')).to_be_visible()
    pg.wait_for_timeout(400)
    pg.get_by_role('button', name='帳本設定').click()
    d4 = pg.get_by_role('dialog')
    d4.get_by_text('不同幣別合併結算').click()
    d4.get_by_role('button', name='儲存').click(); pg.wait_for_timeout(500)
    expect(pg.locator('.ticket .multi-cur .mc-line').first).to_be_visible()
    expect(pg.locator('.ticket')).to_contain_text('各幣別分開')
    shot('20-split-ticket')
    pg.get_by_role('tab', name='結算').click(); pg.wait_for_timeout(300)
    expect(pg.get_by_text('這本帳本各幣別分開結算，不會換匯。')).to_be_visible()
    expect(pg.locator('.cur-section')).to_have_count(2)      # TWD 與 JPY 各一段
    expect(pg.get_by_label('顯示幣別')).to_have_count(0)      # 分開結算不換匯
    shot('21-split-settle')
    pg.get_by_role('group', name='幣別').get_by_role('button', name='¥ JPY').click(); pg.wait_for_timeout(200)
    expect(pg.locator('.cur-section')).to_have_count(1)
    expect(pg.locator('.cur-section > .sec-title').first).to_have_text('JPY 的結算')
    pg.get_by_role('tab', name='統計').click(); pg.wait_for_timeout(300)
    expect(pg.get_by_text('團體總支出')).to_be_visible()
    total_jpy = pg.locator('.stat-total strong').inner_text()
    assert total_jpy.startswith('¥'), total_jpy
    shot('22-split-stats')
    pg.get_by_role('group', name='幣別').get_by_role('button', name='NT$ TWD').click(); pg.wait_for_timeout(200)
    assert pg.locator('.stat-total strong').inner_text().startswith('NT$')
    # 記一筆時不需要匯率
    pg.get_by_role('tab', name='明細').click()
    pg.get_by_role('button', name='記一筆').click()
    d5 = pg.get_by_role('dialog'); d5.get_by_label('幣別').select_option('JPY')
    expect(d5.get_by_text('各幣別分開結算')).to_be_visible()
    expect(d5.get_by_label('匯率')).to_have_count(0)
    pg.keyboard.press('Escape'); pg.wait_for_timeout(300)
    # 改回合併結算
    pg.get_by_role('button', name='帳本設定').click()
    pg.get_by_role('dialog').get_by_text('不同幣別合併結算').click()
    pg.get_by_role('dialog').get_by_role('button', name='儲存').click(); pg.wait_for_timeout(500)
    expect(pg.locator('.ticket .t-amount')).to_be_visible()
    pg.get_by_role('button', name='回帳本列表').click()
    # 深色模式
